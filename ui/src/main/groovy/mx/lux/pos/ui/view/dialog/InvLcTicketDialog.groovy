...
class InvLcTicketDialog extends JDialog implements InvOhListener {
  ...
  protected JComponent composeSelectionPane( ) {
    sb.panel( constraints: BorderLayout.CENTER,
      ...
      cbResumido = checkBox( text: "Resumido", selected: true,
              constraints: "span 2, grow", horizontalAlignment: SwingConstants.CENTER
      )
  ...
  Boolean getResumidoSelected( ) {
    logger.debug( "Get Resumido Selected" )
    Boolean resumido = true
    if ( !this.cbResumido.selected || !this.cbResumido.visible ) {
      resumido = false
    }
        return resumido
  }
  ...
  protected void onBrandValueChanged( ) {
    ...
    if ( false ) {
      ...
      if ( brandModel.isAllSelected() ) {
        Integer nBrand = 0
        for ( String pBrand : ohData.getBrandListLc( idGenre ) ) {
          Integer qtyOh = ohData.getQtyOh( idGenre, asId( pBrand ) )
          qty += qtyOh
          nBrand++
          sb.append( String.format( "\n(%s:%s) Exist:%d", idGenre, pBrand, qtyOh ) )
        }
        logger.debug( sb.toString() )
        this.lblCount.text = String.format( TXT_COUNT_SUMMARY, nBrand, TXT_BRAND_LABEL )
        this.lblTotal.text = String.format( TXT_TOTAL_SUMMARY, qty )
        ...
        Integer nParts = 0
        for ( String partNbr : ohData.getPartListLc( idGenre, brand ) ) {
