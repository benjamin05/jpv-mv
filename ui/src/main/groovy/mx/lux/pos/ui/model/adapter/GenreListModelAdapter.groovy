...
class GenreListModelAdapter implements ComboBoxModel {
  ...
  void setupGenreLc( Collection<Generico> pGenreSet ) {
    this.list.clear()
    for(Generico gen : pGenreSet){
      if( StringUtils.trimToEmpty(gen.id).equalsIgnoreCase(TAG_GENERICO_H) ){
        this.list.add( gen )
      }
    }
    this.syncLc( )
