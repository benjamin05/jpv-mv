...
class CustomerConsultPanel extends JPanel {
    ...
    private ConsultCustomerDialog CustomerAndDialog = null
    ...
    CustomerConsultPanel(Component parent, Customer customer, boolean editar) {
...
//                salutation = comboBox(items: titles*.title, itemStateChanged: titleChanged)
                    ...
                    label('Nombre')
                    firstName = textField(document: new UpperCaseDocument(), editable: false)

                    label('Apellido Paterno')
                    fathersName = textField(document: new UpperCaseDocument(), editable: false)

                    label('Apellido Materno')
                    mothersName = textField(document: new UpperCaseDocument(), editable: false)
...
//                salutation = comboBox(items: titles*.title, itemStateChanged: titleChanged)
                    ...
                    label('Sexo')
                    gender = textField(text: customer.gender.value)

                    label('F. Nacimiento')
                    //dob = spinner( model: spinnerDateModel() )
                    txtBirthDate = textField(text: birthDate, editable: false)
            ...
            panel(border: titledBorder('Contacto'), layout: new MigLayout("wrap 5", "[fill][fill][fill,grow][fill][fill,grow]")) {
                ...
                cbEmail = radioButton( buttonGroup: typeContact, actionPerformed: {principalSelected( TAG_ID_CORREO )},
                        constraints: 'hidemode 3', visible: false )
                label( text: 'Correo:', constraints: 'hidemode 3', visible: false )
                txtEmail = textField( constraints: 'hidemode 3', visible: false )
                arroba = label(text: '@', constraints: 'hidemode 3',  visible: false)
                dominio = comboBox(items: dominios, constraints: 'hidemode 3', visible: false,editable:true )
                ...
                cbCell = radioButton( buttonGroup: typeContact, actionPerformed: {principalSelected( TAG_ID_SMS )},
                        constraints: 'hidemode 3', visible: false )
                label( text: 'Celular:', constraints: 'hidemode 3', visible: false )
                txtSms = textField( constraints: 'hidemode 3', visible: false, document: new UpperCaseDocument() )
                ...
                cbHouse = radioButton( buttonGroup: typeContact, actionPerformed: {principalSelected( TAG_ID_TELEFONO )},
                        constraints: 'hidemode 3', visible: false )
                label( text: 'Telefono:', constraints: 'hidemode 3', visible: false )
                txtTelefono = textField( constraints: 'hidemode 3', visible: false, document: new UpperCaseDocument() )
                label( text: ' ', constraints: 'hidemode 3', visible: false )
                label( text: ' ', constraints: 'hidemode 3', visible: false )
            ...
            panel(border: titledBorder('Dirección'), layout: new MigLayout('wrap 3', '[][fill,grow][]'), constraints: 'hidemode 3') {
                ...
                primary = textField(document: new UpperCaseDocument(), constraints: 'span 2', editable: false)

                label('C.P.')
                zipcode = textField(editable: false, constraints: 'span 2')
                //button('Buscar', actionPerformed: doSearch)

                label('Colonia')
                locationField = textField( editable: false, constraints: 'span 2')

                label('Delegación/Mnpo')
                city = textField(editable: false, constraints: 'span 2')

                label('Estado')
                stateField = textField(editable: false, constraints: 'span 2')
            }
...
//            bean(salutation, selectedItem: bind(source: customer, sourceProperty: 'title', mutual: true))
            ...
            //bean(gender, text: bind(source: customer, sourceProperty: 'gender', mutual: true))
            gender.text = customer.gender.value
            bean(primary, text: bind(source: customer.address, sourceProperty: 'primary', mutual: true))
            bean(stateField, text: bind(source: customer.address, sourceProperty: 'state', mutual: true))
            bean(city, text: bind(source: customer.address, sourceProperty: 'city', mutual: true))
            bean(locationField, text: bind(source: customer.address, sourceProperty: 'location', mutual: true))
            bean(zipcode, text: bind(source: customer.address, sourceProperty: 'zipcode', mutual: true))
