...
class EditDepositDialog extends JDialog {
  ...
  private void buildUI( Component parent ) {
    sb.dialog( this,
        ...
        resizable: true,
        preferredSize: [350 , 140],
        pack: true,
        modal: true,
        layout: new MigLayout( 'fill,wrap', '[fill]' )
    ) {
      panel( layout: new MigLayout( 'wrap 2', '[left][fill,grow]' ) ) {
        /*label( 'No.' )
  ...
  private boolean isValid( Deposit deposit ) {
    ...
    if ( StringUtils.isBlank( ammount.text ) ) {
      ...
      Double ammount = 0.00
      try{
        ammount = NumberFormat.getInstance().parse(this.ammount.text).doubleValue()
  ...
  private boolean fillDeposit( ) {
    deposit.enterDate = new Date()
    deposit.depositDate = new Date()
    deposit.depositType = 'EFECTIVO'
    deposit.bankId = Registry.depositBank
    deposit.reference = ''
  }
