...
import mx.lux.pos.ui.model.MultypaymentActive
...
class OrderActiveMultypaymentDialog extends JDialog {
  ...
  private List<MultypaymentActive> orderList
  private MultypaymentActive selection
  ...
  private OrderPanel orderPanel
    ...
    OrderActiveMultypaymentDialog( OrderPanel orderPanel ) {
    this.orderList = new ArrayList<MultypaymentActive>()
          ...
          tOrders = table( selectionMode: ListSelectionModel.SINGLE_SELECTION,
            ...
            model = tableModel( list: orderList ) {
              closureColumn( header: TXT_CUST_NAME_LABEL,
                  minWidth: 240,
                  read: { MultypaymentActive o -> o.customerName }
              )
              closureColumn( header: TXT_PARTS_LABEL,
                  minWidth: 180,
                  read: { MultypaymentActive o -> o.partList }
              )
              closureColumn( header: TXT_AMOUNT_LABEL,
                  maxWidth: 100,
                  read: { MultypaymentActive o -> String.format( '$%,.2f', o.amount) }
              )
  ...
  void setCustomerList( List<ClienteProceso> pCustomerList ) {
    ...
    for (ClienteProceso c : pCustomerList) {
      List<NotaVenta> lstNotasValidas = new ArrayList<>()
      List<NotaVenta> notas = OrderController.findOrderByClient( c.idCliente )
      this.orderList.add( new MultypaymentActive(notas, c.cliente))
    }
  ...
  // Triggers
    private def doSelectOrderClick = { MouseEvent ev ->
        if ( SwingUtilities.isLeftMouseButton( ev ) ) {
            MultypaymentActive selection = ev.source.selectedElement as MultypaymentActive
            if ( ev.clickCount == 2 && selection?.customer.id ) {
                onSelection()
            }
        }
    }
  ...
  protected void onSelection( ) {
    ...
    if (tOrders.selectedRowCount > 0) {
      ...
      this.setVisible( false )
      List<Order> lstOrders = OrderController.castListNotaVentaToOrder( selection.order )
      MultypaymentDialog mpDialog = new MultypaymentDialog( selection.customer.id, lstOrders.get(0), lstOrders.get(1), orderPanel )
      mpDialog.activate()
      dispose()
    } else {
