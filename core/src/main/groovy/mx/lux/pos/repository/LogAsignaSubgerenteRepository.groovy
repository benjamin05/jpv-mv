...
interface LogAsignaSubgerenteRepository extends JpaRepository<LogAsignaSubgerente, Integer>, QueryDslPredicateExecutor<LogAsignaSubgerente> {
  ...
  @Query( value = "SELECT * FROM log_asigna_subgerente WHERE (fecha_inicial IS NOT NULL AND fecha_inicial >= CAST(NOW() AS DATE)) OR (fecha_inicial IS NULL AND fecha >= CAST(NOW() AS DATE))", nativeQuery = true)
